...
\begin{definition}[1-Form des Differential]
%% von http://www.math.purdue.edu/~dvb/preprints/diffforms.pdf
...
\subsection{Bestimmung eines Potentials durch Raten}
In den Nachfolgenden Kapitel beschriebener Vorgang ist teilweise nicht nötig. In gewissen Fällen kann das Potential $f$ erraten werden in dem wir das Vektorfeld $\vec{v}$ partielle Integrieren und dann eine Funktion $f$ erraten, so dass $\nabla f$ wieder $\vec{v}$ ergibt.
...
Sei eine Kurve in der Parameterdarstellung (z.B.: $x(t) =sin(2t)$ und $y(t)=cos(3t)$ über $0\leq t \leq \pi$) gegeben. So ist die Fläche $A = \int_0^\pi y\dot{x} \, dt$.
